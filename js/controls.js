...
L.Control.Menu = L.Control.extend({
    onAdd: function (map) {
        ...
        var parametersContent = parametersTab(this.state);
        var districtsContent = districtsTab();
        var paramsTab = createTab(nav, "Constraints", parametersContent, "params", true);
        var distTab = createTab(nav, "Top Districtings", districtsContent, "districts");
...
function incumbentsContent(state){
    console.log(statesObj[state]['reps']);
...
function modalDialog(id,headerText,bodyContent){
    ...
    var closeBtn = createButton(footer, 'button','Close','btn btn-secondary');
